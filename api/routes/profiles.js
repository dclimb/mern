...
router.post('/', passport.authenticate('jwt', {session: false}), (req,res) =>{
  ...
  Profile.findOne({user: req.user.id }, (err,profile) => {
    if (profile){
      //UPDATE
      Profile.findByIdAndUpdate(profile.id, {$set:{profileFields}}, {new:true})
             .then( update =>{
...
router.delete('/experience/:exp_id',
  ...
  Profile.findOne({user: req.user.id}, (err, profile) =>{
    ...
    const findIndex = profile.experience.map( item => item.experience)
...
router.delete('/education/:edu_id', passport.authenticate('jwt', {session: false}), (req,res) =>{
  ...
  Profile.findOne({user: req.user.id}, (err, profile) =>{
    ...
    const findIndex = profile.education.map( item => item.education)
                             .indexOf(req.params.exp_id);
